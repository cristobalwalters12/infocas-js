  ...
  <div>
    ...
    <v-card>
      ...
      >
        ...
        <div class="d-flex">
          <div>
            <v-btn color="pink-darken-4" @click="$router.push('/respaldosPresionDiferencial')"
      ...
      <v-table height="400px">
        ...
        <tbody>
          <tr v-for="item in data" :key="item.id">
            ...
            <td class="text-center">{{ item.sizeFormatted }}</td>
...
import { getControladoresPredif } from '../../../api/services/sensoresPreDifServices'
...
const fetchControladores = async (controlador) => {
  loading.value = true
  try {
    const response = await getControladoresPredif({ controlador })
...
const verRespaldo = async (item) => {
  await router.push(`/ArchivoGatewayPreDif/${props.id}/${item}`)
}
