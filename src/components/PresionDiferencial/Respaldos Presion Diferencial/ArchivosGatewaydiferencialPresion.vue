  ...
  <div>
    ...
    <v-card>
      ...
      >
        ...
        <div class="d-flex">
          <div>
            <v-btn color="pink-darken-4" @click="$router.push(`/txtRespaldosPreDif/${id}`)"
...
const fetchArchivosGateway = async (controlador, carpeta) => {
  ...
  try {
    const response = await getArchivosControladorPredif(request)
...
const handleDownload = async (archivo) => {
  ...
  try {
    await descargatxtGatewayPredif(respaldo)
  } catch (error) {
